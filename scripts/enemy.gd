...
# Custom behaviors for VS style enemies
...
func _physics_process(delta: float) -> void:
	...
	if enemy_type == "bat":
		...
		elif werewolf_charge_cooldown <= 0.0 and global_position.distance_to(player.global_position) <= 160.0:
	...
	# "Boiling Horde" clumping detection (tight proximity check to other enemies)
	var close_count := 0
	var my_pos := global_position
	for other in get_tree().get_nodes_in_group("enemy"):
		if other != self and is_instance_valid(other) and not other.dying:
			if my_pos.distance_squared_to(other.global_position) < 900.0: # 30 pixels (overlapping/clumped)
				close_count += 1
				if close_count >= 2:
					break
